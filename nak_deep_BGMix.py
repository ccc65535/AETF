...
from baseline.deepnet import  Deep4Net
...
# log_file_name='./record/log-deep-'+time.strftime('%m-%d-%H-%M')
# log_file=open(log_file_name,mode='w+',buffering=-1)d

res_file_name='./record/res-deep-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
...
for duration in durations:
    ...
    # for k in [3]:
        ...
        save_path='./model_save/deep-fold-'+str(k)+'-'+str(duration)+'s-naki.pth'
        tunning=True
        new_test=True
        # new_test = False
        if tunning:
        ...
        # for sub_id in [2,9,10]:
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/naki-deep-'+str(duration)+'s-fold-'+str(k)+'-sub-'+str(sub_id)+'.pth'
