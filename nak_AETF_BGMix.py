...
from metabci.brainda.datasets import Nakanishi2015  # type: ignore
...
dataset_channels= ['PO7', 'PO3', 'POZ', 'PO4', 'PO8', 'O1', 'OZ', 'O2']


srate = 250
durations = [ 0.3, 0.4, 0.5]
# durations = [0.2,0.25,0.3,0.35,0.4,0.45,0.5]
# durations = [.5]
# durations = [.25,.35,.45]
...
for duration in durations:
    ...
    # for k in [3]:
        ...
        ly=int(indicator*2)
        df=int(np.power(2,indicator+4))


        # dim_feedforward,n_layers,n_freq_filters=128,2,8
        n_spatial_filters=8
        n_freq_filters=n_classes
        dim_feedforward,n_layers=2*n_classes,2
        batch_size,max_epochs,lr = 128,600,1e-4
        dropout1,dropout2=0.4,0
        ...
        save_path='./model_save/temp_all-fold-'+str(k)+'-'+str(duration)+'s-naki.pth'
        tunning=True
        # new_test=True
        new_test = False
        if tunning:
        ...
        # for sub_id in [1,2]:
            ...
            sub_trainX,sub_trainY=BGMix.generate_data(used_X.numpy(),used_Y,nt=[2 for i in range(40)]+[3 for i in range(40)])

            sub_validateX, sub_validateY =used_X,used_Y

            sub_testX=sub_testX.to(device)
         
            

            batch_size,max_epochs,lr = 256,400,5e-4
            # lamda=0.8
            patience=10
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/naki-model-'+str(duration)+'s-fold-'+str(k)+'-sub-'+str(sub_id)+'.pth'
            ...
            for epoch in range(max_epochs):
                ...
                with torch.no_grad():
                    ...
                    pts=f'epoch:{epoch},train loss:{total_loss/(len(data_loader)):.3f},valid loss:{valid_loss:.4f},valid acc:{valid_acc:.3f},test acc:{sub_acc:.3f}'
                    print(pts)
                    log_file.write(pts+'\n')
                    if (valid_loss+5e-4<min_loss):
                    # if (valid_loss+5e-4<min_loss) or(valid_acc>max_acc+1e-3):
                        ...
                        min_loss=np.min([valid_loss.detach().cpu().numpy(),min_loss])
