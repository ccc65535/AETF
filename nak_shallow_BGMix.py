...
from  metabci.brainda.algorithms.utils.model_selection import (# type: ignore
    set_random_seeds, match_loo_indices)


from sklearn.metrics import confusion_matrix, balanced_accuracy_score,accuracy_score

from baseline.shallownet import  ShallowNet
...
# log_file_name='./record/log-shallow-'+time.strftime('%m-%d-%H-%M')
# log_file=open(log_file_name,mode='w+',buffering=-1)

res_file_name='./record/res-shallow-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
...
for duration in durations:
    ...
    # for k in [3]:
        ...
        all_model=ShallowNet(n_channels, n_samples, n_classes,
                    n_time_filters=40,
                    time_kernel=15,
                    n_space_filters=40,
                    pool_kernel=5,
        ...
        save_path='./model_save/shallow-fold-'+str(k)+'-'+str(duration)+'s-naki.pth'
        tunning=True
        new_test=True
        # new_test = False
        if tunning:
        ...
        # for sub_id in [2,9,10]:
            ...
            sub_validateX,sub_validateY=used_X,used_Y_zip
            ...
            batch_size,max_epochs,lr = 128,600,1e-2
            # lamda=0.8
            patience=10
            ...
            sub_model=ShallowNet(n_channels, n_samples, n_classes,
                    n_time_filters=40,
                    time_kernel=15,
                    n_space_filters=40,
                    pool_kernel=5,
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/naki-shallow-'+str(duration)+'s-fold-'+str(k)+'-sub-'+str(sub_id)+'.pth'
